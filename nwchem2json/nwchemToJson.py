...
class nwchemToJson:
  ...
  def __init__(self, processOrbitals=True):
    self.simulationEnv = {}
  ...
  def genFuncForMol(self):
    ...
    for atom in self.molecule.molecule['atoms']:
      ...
      for basis in self.basis.basis['basisFunctions']:
        if (basis['elementLabel'].lower() == atom['elementLabel'].lower()
            and basis['basisSetType'] == 'orbitalBasis'):  # noqa: W503
  ...
  def reorderFunctionsInMO(self, coefficientsIn):
    ...
    for atom in self.molecule.molecule['atoms']:
      for basis in self.basis.basis['basisFunctions']:
        if (basis['elementLabel'] == atom['elementLabel']
            and basis['basisSetType'] == 'orbitalBasis'):  # noqa: W503
          spherical = basis['basisSetHarmonicType'] == 'spherical'
          for contr in basis['basisSetContraction']:
  ...
  def readScfDft(self, line, streamIn):
    ...
    def alphaElec(line):
      self.calcSetup['numberOfElectrons'] = (
          int(line.split()[3]) + int(self.calcSetup['numberOfElectrons']))
      ...
      for i in range(loopRange):
        ...
        while line:
          if line.find('Vector') >= 0:
            ...
            orbital['id'] = 'molecularOrbital{}.{}.Mol.{}'.format(
              spinLabel, str(vars[1]), str(self.molecule.molCount))
    ...
    while line:
      if line.find('Task  times') >= 0 and not self.subTask:
        ...
        break
      elif ((line.find('Module') >= 0
             or line.find('Parallel integral file') >= 0  # noqa: W503
             or line.find('Line search') >= 0  # noqa: W503
             or line.find('Saving state') >= 0)  # noqa: W503
            and self.subTask):  # noqa: W503
        break
  ...
  def readProp(self, line, streamIn):
    ...
    def propSpinSpin(streamIn):
      myList = ['Fermi Contact Term', 'Spin-Dipole Term', 'Fermi Contact - Spin-Dipole Cross Term',
                'Paramagnetic Spin-Orbit Term', 'Diamagnetic Spin-Orbit Term',
      ...
      while line:
        if line.find('Atom') >= 0:
          ...
          coupledAtom1 = {
            'atom': 'Atom.' + str(atom2) + '.Mol.' + str(self.molecule.molCount),
            'atomicWeight': float(vars[6].split('-')[0])
          }
          coupledAtom2 = {
            'atom': 'Atom.' + str(atom1) + '.Mol.' + str(self.molecule.molCount),
            'atomicWeight': float(vars[2].split('-')[0])
          }
          line = streamIn.readline()
          vars = streamIn.readline().split()
  ...
  def readTce(self, line, streamIn):
    ...
    if perturbative:
      ...
      self.calcSetup['waveFunctionTheory'] += (' with ' + streamIn.readline().split(':')[1]
                                               + ' perturbative correction')  # noqa: W503
    self.calcTask['calculationSetup'] = self.calcSetup
    while line:
  ...
  def readEcp(self, line, streamIn):
    ...
    while line:
      if (line.find('Module') >= 0
          or line.find('library') >= 0  # noqa: W503
          or line.find('NWChem') >= 0  # noqa: W503
          or emptyLine > 1):  # noqa: W503
        break
      vars = streamIn.readline().split()
